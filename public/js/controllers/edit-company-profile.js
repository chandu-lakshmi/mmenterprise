(function () {
    ...
    function editCompanyProfileController($scope, httpService, pendingRequests, detectBrowser,  $window, CompanyDetails, App) {
        ...
        function numberOfEmployees(count) {
            if (count >= 10 && count <= 50)
                vm.company_details.number_of_employees = vm.no_of_emp[0];
            else if (count >= 50 && count <= 100)
                vm.company_details.number_of_employees = vm.no_of_emp[1];
            else if (count >= 101 && count <= 500)
                vm.company_details.number_of_employees = vm.no_of_emp[2];
            else if (count >= 501 && count <= 1000)
                vm.company_details.number_of_employees = vm.no_of_emp[3];
            else if (count >= 1001 && count <= 5000)
                vm.company_details.number_of_employees = vm.no_of_emp[4];
            else
                vm.company_details.number_of_employees = vm.no_of_emp[5];
        }
